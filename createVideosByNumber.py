...
def generate_video(video_folder, audio_folder, text_file, fonts, logo_file, channel_name, number):
    ...
    for i in range(number):
        ...
        verse_text_clip = (vc.TextClip(text_verse, color='white', font=selected_font, size=(900, 450), method="caption",
                                       align="center")
        ...
        text_source = text_source.replace(":", "")
        text_source = text_source.rstrip('\n')


        # Save the final video to E:/Bots/VideoMaker/customers/{channel_name} in format: text-video-audio-font.mp4
        final_clip.write_videofile(f"E:/Bots/VideoMaker/customers/{channel_name}/{i}-{text_source}-{random_video_num}-{random_audio_num}-{random_font_num}.mp4",
                                   threads=8,
                                   fps=24,
                                   codec="libx264",
                                   audio_codec="aac")
        ...
        audio_clip.close()
        verse_text_clip.close()
        source_text_clip.close()
        logo_image_clip.close()
        final_clip.close()
